...
"""
# 通过对列表的操作，将多个文件合并
# 相比于27_g(r)+CN数据整合，28主要是把标题改了，因为title = line.split( )[4] + line.split( )[5] # @    s0 legend  "% Angle [B-O-B]" 
# Angles [O -B -O ]/Angles [O -Si-O ]/Angles [Si-O -Si]/Angles [Si-O -B ]
...
print("-------------")
print("如果需要为Angle曲线进行梯度增高变换，请输入梯度值，如3,将按照0*3,1*3,2*3,3*3等进行增高;如不需要，则输入0")     # 提示命令行输入
nu = float(input())              # 注意字符串输入变量的数据类型转换


print("请按顺序依次输入需要批量处理的文件名,用/号分隔，ISAACS导出的 Angles [B -O -B ]数据文件格式")     # 提示命令行输入
...
#            x_f = "x"+ str(i)
#            y_f = "y"+ str(i)            
            if pp == 16:           #第16行的格式为: @    s0 legend  "% Angle [B-O-B]" 
                title = line.split( )[4] + line.split( )[5] # @    s0 legend  "% Angle [B-O-B]" 
    #            title1 = title[1]+title[-10:-2]#
                list2[2*j].append("angle")
