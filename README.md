...
注意：运行上述脚本`功能02`会提示输入原子顺序，以便生成的data文件中按照该顺序排列原子种类。新增支持输出 atomic 原子格式的 lammps data 文件，在功能2运行过程中会提示选择 charge 格式还是 atomic 格式，atomic 格式是deepmd运行lammps支持的标准格式。
