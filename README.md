...
2. 团簇结构提取思路1：
   - 先使用`方法18`基于原子对截断半径`B O 4.5 B Si 3.4 B B 3.18 B Al 3.5`提取**某个中心原子**（如B原子）的多帧xyz文件，注意B-O原子对截断半径是RDF的第二个谷值，对应B-O第二截断半径，其余B-Si，B-B，B-Al都是第一截断半径。获取的多帧xyz轨迹文件，是某个中心原子对应的粗糙团簇结构文件。该粗糙团簇最外层的部分O原子是多余的，但是其余原子都是准确的。
