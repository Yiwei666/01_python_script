...
2. `方法17和方法18`局域配位结构算法分析
   - 代码核心思路：将所有帧中的所有原子（非扩增后盒子的原始原子，减少计算和内存占用）都写入到字典中，最外层for循环是帧数循环，然后次外层是每一帧中的所有j原子循环。接下来需要判断循环的原子类型（中心i原子？非配位原子？配位原子j？）。最里面的一层循环是针对每一个中心原子i，每一帧中的每一个配位原子j是否满足截断半径距离要求（距离计算需要根据周期性条件进行变换）。如果满足要求，`方法17`会写入该j配位原子的原始真实坐标，且只会写入一次，无论该j配位原子是否满足多个中心原子的截断半径要求；`方法18`可能会在多个中心原子对j配位原子的距离循环判断中，写入多个j原子的坐标，且这些坐标都不同。
