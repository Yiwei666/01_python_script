...
2. 局域配位结构算法分析
   - 代码核心思路：将所有帧中的所有原子都写入到字典中，最外层for循环是帧数循环，然后次外层是每一帧中的所有j原子循环。接下来需要判断循环的原子类型（中心i原子？非配位原子？配位原子j？）。最里面的一层循环是针对每一个中心原子i，每一帧中的每一个配位原子j是否满足要求。
...
2. 团簇结构提取思路1：
   - 先使用`方法18`基于原子对截断半径`B O 4.5 B Si 3.4 B B 3.18 B Al 3.5`提取某个B原子的多帧xyz文件，注意B-O原子对截断半径是RDF的第二个谷值，对应B-O第二截断半径，其余B-Si，B-B，B-Al都是第一截断半径。获取的多帧xyz轨迹文件是粗超的团簇结构文件，因为最外层的部分O原子是多余的，但是其余原子都是准确的。
   - 再使用`方法20`基于原子对第一截断半径`B O 2.2 Si O 2.2 Al O 2.5`提取精细团簇结构，去掉多余的O，`B，Si，Al`在提取过程中，不会发生删减。获取的多帧xyz轨迹文件是精细的团簇结构。
...
3. 团簇结构提取思路2：
   ...
   - 再使用`方法18`基于中心原子编号和第二截断半径`B O 4.5 B Si 3.4 B B 3.18 B Al 3.5`，读取该帧xyz文件，提取该中心原子对应的精细团簇结构。
   - 这种方法不需要记录精细团簇结构中所有中心原子和配位原子的编号，效率较高。
