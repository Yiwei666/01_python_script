...
# 查找包含 'origin_dn(r)' 字符串的文件
file_list = glob.glob("*origin_dn(r)*")

# 如果没有找到符合条件的文件，提示用户并退出
if not file_list:
    print("未找到文件名中包含 'origin_dn(r)' 的文件。")
    exit()
...
for file in selected_files:
    ...
    # 筛选 x 在 0 到 6 范围内的数据
    filtered_data = data[(data['x'] >= 0) & (data['x'] <= 5)]
...
plt.xlabel('X')
plt.ylabel('Y')
plt.title('Curve for selected origin_dn(r) files')
plt.legend()
plt.show()
