...
class txtPlot():
    def __init__(self,fileName,nStart,saveTxt):  # 默认 nStart = 2，代表起始行数
        ...
        with open(self.fileName, 'r') as f:
            lines = f.readlines()
            self.txtList = lines                       # txt数据文件所有行写成一个列表，包括表头
            columnTotal = self.txtList[self.nStart-1].split( )      # 默认多个空格作为分隔符
            # print("第一行数据",columnTotal)
            self.columnTotal = len(columnTotal)        # 列数总和,即计算起始行数的列数
            # columnIni = 1                            # 将列数的起始值设为1
            for i in range(1,self.columnTotal+1):
                columnDict[i] = []
            # print("列数据字典初始化",columnDict)
            for i,j in enumerate(self.txtList):
                if i >= self.nStart-1:                 # 行数减去1即为索引index，从数据行的索引开始
                    for m,n in enumerate(j.split( )):
