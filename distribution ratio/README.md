...
在使用ISAACS分析硅-硅酸盐两相从头算输出的原子轨迹时，为了计算某种原子X的局域结构，通常会设置 X-O 以及 X-Si 截断半径，使其输出的配位结构中只包含这两类原子，从而基于这两类原子的相对比例判断其所属相
...
2. 编程思路
   - 初始版本，忽略空行和 # 开头行
   - 筛选并打印第二列数据为0的行，通常需要注意第二列数据的原子种类，判断是否为Si或者O
...
### 1.2 `02_界面及体相配位结构分析(isaacs数据后处理).py`
...
2. 编程思路
   - 筛选位于两相以及界面的行数，可自定义Si和O种类原子所在的列数。
   - 对每一部分的原子数以及百分比进行求和，即倒数第二列、四列数据的求和
   - 按照倒数第二列数据递减的顺序打印
...
2        2       0       0       0           0.03311   or       0.414 %
3        3       0       0       0           0.15232   or       1.904 %
请依次输入O配位数 和 Si配位数 所在列数，用英文逗号隔开，列数从1开始计数: 2,3
First number: 2
Second number: 3
筛选X-O配位数为0的行，位于硅相
筛选满足要求的行
...
计算界面及两相的杂质原子占比，并对界面原子进行分割

2. 编程思路
   - 提示输入 "请输入O配位数所在列数，Si配位数所在列数，渣中X-O平均配位数，硅中X-Si平均配位数，用英文逗号隔开"
   - 返回位于硅相，硅酸盐相以及界面处的行，相应X原子数以及百分比
   - 返回考虑界面分割后的硅相X原子数以及渣相X原子数
...
2        2       0       0       0           0.03311   or       0.414 %
3        3       0       0       0           0.15232   or       1.904 %
请输入O配位数所在列数，Si配位数所在列数，渣中X-O平均配位数，硅中X-Si平均配位数，用英文逗号隔开：2,3,3.87,3.78
[2.0, 3.0, 3.87, 3.78]
【筛选O配位数为0列数，即位于Si相】
筛选满足要求的行
4        0       4       0       0           0.63576   or       7.947 %
...
【筛选O配位数以及Si配位数都不为0的列，即位于界面处】
筛选满足要求的行
2        1       1       0       0           0.54967   or       6.871 %  BO_ratio: 49.412 %   N_B_in_slag: 0.272   BSi_ratio: 50.588 %   N_B_in_Si: 0.278
4        3       1       0       0           0.70199   or       8.775 %  BO_ratio: 74.556 %   N_B_in_slag: 0.523   BSi_ratio: 25.444 %   N_B_in_Si: 0.179
5        1       4       0       0           0.02649   or       0.331 %  BO_ratio: 19.626 %   N_B_in_slag: 0.005   BSi_ratio: 80.374 %   N_B_in_Si: 0.021
3        2       1       0       0           0.46358   or       5.795 %  BO_ratio: 66.142 %   N_B_in_slag: 0.307   BSi_ratio: 33.858 %   N_B_in_Si: 0.157
4        1       3       0       0           0.10596   or       1.325 %  BO_ratio: 24.561 %   N_B_in_slag: 0.026   BSi_ratio: 75.439 %   N_B_in_Si: 0.08
3        1       2       0       0           0.07947   or       0.993 %  BO_ratio: 32.812 %   N_B_in_slag: 0.026   BSi_ratio: 67.188 %   N_B_in_Si: 0.053
4        2       2       0       0           0.00662   or       0.083 %  BO_ratio: 49.412 %   N_B_in_slag: 0.003   BSi_ratio: 50.588 %   N_B_in_Si: 0.003
5        3       2       0       0           0.11258   or       1.407 %  BO_ratio: 59.434 %   N_B_in_slag: 0.067   BSi_ratio: 40.566 %   N_B_in_Si: 0.046
倒数第二列数据的求和： 25.579999999999995  %
倒数第四列数据的求和： 2.04636
界面分配到硅相原子数sum_N_B_in_Si： 0.81728
界面分配到渣相原子数sum_N_B_in_slag： 1.22908
界面原子总数sum_N_B_in_Si + sum_N_B_in_slag： 2.04636
按照倒数第二列数据递减的顺序打印：
4 3 1 0 0 0.70199 or 8.775 %
2 1 1 0 0 0.54967 or 6.871 %
3 2 1 0 0 0.46358 or 5.795 %
...
考虑界面分割后位于硅相的X总原子数： 5.76821 + 0.8172792082471557 = 6.585
考虑界面分割后位于渣相的X总原子数： 0.18543 + 1.2290807917528443 = 1.415
```




### 1.4 `06_X-O两相平均配位数及杂质原子两相分布和平均氧化程度k计算.py`

1. 功能

基于ISAACS输出的包含有X-O和X-Si配位信息的txt文件，计算两相共存体系中X的平均X-O配位数，X的平均氧化程度，分配到硅相和硅酸盐相中的X原子数量


2. 编程思路
   - 请输入要处理的文件名：V_twophase.txt
   - 请输入氧原子所在的列，从1开始计数，氧(O)原子通常位于第2列: 2
   - 请输入浮点数：硅酸盐中X-O平均配位数y_CN_SilicateAve, 两相中总的杂质原子数z_total_Nx（用英文逗号分隔）: 4.32,8
...
2        2       0       0       0           0.03311   or       0.414 %
3        3       0       0       0           0.15232   or       1.904 %
合并后的第1列，相应倒数第4列，第2列数据
0.0 5.76821 72.102 %
1.0 0.7615900000000001 9.52 %
3.0 0.96689 12.086 %
2.0 0.50331 6.292 %
X-O两相平均配位数： 0.58362
请输入浮点数：硅酸盐中X-O平均配位数y_CN_SilicateAve, 两相中总的杂质原子数z_total_Nx（用英文逗号分隔）: 3.87,8
进入到silicate中的X原子数： 1.2064
留在silicon中的X原子数： 6.7936
X两相平均氧化程度，k值，即X-O两相配位数/硅酸盐X-O配位数: 0.1508
```
