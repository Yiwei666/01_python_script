...
二维高斯函数叠加形成的曲面在xz平面的投影并不是简单的各个二维高斯函数在xz平面投影的加和。计算叠加后曲面的投影可以这样考虑，首先在x轴上类似 `x = np.linspace(-1, 4, 100)` 确定步长，然后针对每一个确定的x，遍历 `y = np.linspace(0, 5, 100)` 中的每一个步长，找到 确定x时的最大的z，将其作为在xz平面上的投影点
