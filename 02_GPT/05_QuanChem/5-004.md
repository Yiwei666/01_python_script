...
3. VMD脚本和配置：
   ...
   - 可选：将VMD安装目录写入到系统环境变量中，然后将`showcub.vmd`拷贝到cub文件所在目录，使用cmd命令行启动vmd，使用`cub EDD 0.0004`命令来可视化EDD.cub文件，其中等值面设置为0.0004。
...
- 导出函数数据为func2.cub和func1.cub文件。
- 将RDGfill.vmd文件拷贝到cub文件所在目录。
- 启动VMD，输入`source RDGfill2.vmd`执行绘图脚本。
