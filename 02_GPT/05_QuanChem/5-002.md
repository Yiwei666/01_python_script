...
1. 对于基于实空间函数的波函数分析，如电子密度、ELF、静电势、AIM和RDG分析，使用6-31G*基组通常已足够，6-311G**或def-TZVP基组更为充足，弥散函数对这些分析影响不大且会浪费计算时间；而对于CDA、Mayer键级、Wiberg键级、Mulliken分析和SCPA分析，不应使用带有弥散函数的基组，使用较大的基组也不会改善结果，甚至可能变差。参考：http://sobereva.com/336
