...
def plot_fes(datafile, cmap='viridis', levels=40, offset=-25, figsize=(10,6)):
    ...
    y = data[:, 1]
    # z = data[:, 2]
    # z = data[:, 2]*4.3597*6.022*100 # 1 Hartree = 4.3597482e-21 KJ, 1 mol = 6.022*E23   unit：KJ/mol
    z = data[:, 2]*27.2113966 # 1 Hartree = 27.211396641308 eV   unit：eV
    ...
    # 添加标签和标题
    ax.set_xlabel('B-Si coordination number')
    ax.set_ylabel('B-O coordination number')
    # ax.set_zlabel('Energy (kJ/mol)',labelpad=11)  # 设置与z轴距离
    ax.set_zlabel('Energy (eV)',labelpad=11)
    ...
    # ax.set_zlim(-13000, 0)
    ax.set_zlim(offset, 0)  # 将z轴方向绘制填色图的界面z坐标设置为z轴的最小值
    ...
    cbar_ax = fig.add_axes([0.81, 0.10, 0.02, 0.7])
...
# 07函数 计算无偏分子动力学反应坐标上的自由能面
# import numpy as np
# import matplotlib.pyplot as plt
def calculate_fes(colvar_path, temperature, bins=50):
    ...
    # Free energy surface
    fes = -kb * temperature * np.log(prob)  #  #Change temperature according to your MD simulations!
...
if __name__ == '__main__':
  ...
  本脚本的功能如下:
      ...
      06: 绘制3列（x,y,z）数据组成的曲面图及其二维填色图，注意修改z轴，填色图纵坐标
      07: 计算无偏分子动力学反应坐标上的自由能面
    ...
    if  defChoose == "01":
        ...
        filename = inputFunction()
        # plot_fes(filename, cmap='viridis', levels=40, offset=-20000, figsize=(10,6))
        plot_fes(filename, cmap='viridis', levels=40, offset=-200, figsize=(10,6))
        ...
        colvar_path = "Si_silicate-COLVAR.metadynLog"
        # colvar_path = "./SiTiB-COLVAR.metadynLog"
        fes = calculate_fes(colvar_path, temperature = 1823, bins=10)
